const generateSubCatalog = () => {
    ...
    const updateHTML = (header, list) => {
        ...
        let listSubCatalog = '';
        list.forEach(elem => {
            listSubCatalog += `<li class="subcatalog-list__item"><a href="goods?filial=${header}&subcat=${elem}">${elem}</a></li>`
        });
        const subCatalogHTML = `
                ...
                <h2 class="subcatalog-header"><a href="goods?cat=${header}">${header}</a></h2>
                <ul class="subcatalog-list">${listSubCatalog}</ul>`;
